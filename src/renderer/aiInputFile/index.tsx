...
import type { BaseSchema, FormBaseControl, SchemaApi } from 'amis';
...
export interface FormBaseControlSchema extends FormBaseControl {
   ...
   */
  remark?: string | Omit<RemarkSchema, 'type'>;

  /**
   * 显示一个小图标, 鼠标放上去的时候显示提示内容, 这个小图标跟 label 在一起
   */
  labelRemark?: string | Omit<RemarkSchema, 'type'>;
}
...
export default class FileControl extends React.Component<FileProps, FileState> {
  ...
  componentDidUpdate(prevProps: FileProps, preState) {
  ...
  render() {
    ...
    return (
      <div className={cx('FileControl', className)}>
        ...
        {Array.isArray(files) ? (
          <ul className={cx('FileControl-list', 'file-control-list')}>
            {files.map((file, index) => {
              const filename =
                file[nameField as keyof typeof file] ||
                (file as FileValue).filename ||
              ...
              return (
                <li key={file.id}>
                  <TooltipWrapper
                    ...
                    tooltip={
                      {
                        content:file.state === 'invalid' || file.state === 'error'
                        ? (file as FileValue).error ||
                        (maxSize && file.size > maxSize
                          ? __('File.maxSize', {
                            filename: file.name,
                    ...
                    >
                      ...
                      {!disabled ? (
                        <a
                          data-tooltip={__('Select.clear')}
                          data-position="right"
                          className={cx('FileControl-clear', 'file-control-clear')}
                          onClick={() => this.removeFile(file, index)}
                        >
                          <Icon icon="close" className="icon" />
                        </a>
